...
class form_processor {
 ...
 public:
  ...
  static form_processor* load_instance(binary_decoder& data);
  void save_instance(binary_encoder& enc);
  template <class T> class registrator { public: registrator() { factory()[T().name()] = []()->form_processor*{ return new T(); }; }};

 private:
  typedef unordered_map<string, function<form_processor*()>> factory_map;
